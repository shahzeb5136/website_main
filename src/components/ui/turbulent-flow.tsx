...
export const TurbulentFlow: React.FC<TurbulentFlowProps> = ({
  children,
  className = "",
}) => {
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Nuclear-themed color palette: amber, golden, warm tones
        vec3 color1 = vec3(1.0, 0.6, 0.05);  // Golden amber
        vec3 color2 = vec3(0.95, 0.4, 0.0);   // Deep orange
        vec3 color3 = vec3(0.6, 0.2, 0.0);    // Burnt sienna
        vec3 color4 = vec3(1.0, 0.85, 0.2);   // Bright gold
        vec3 color5 = vec3(0.9, 0.3, 0.1);    // Fire red
        ...
        finalColor += (microTurbulence * noiseDetail) * 0.3;
    ...
    tl.to(material.uniforms.u_turbulence, {
      ...
      .to(
        material.uniforms.u_noise_scale,
        {
          value: 6.0,
          duration: 8,
          ease: "power2.inOut",
      ...
      .to(
        material.uniforms.u_distortion,
        {
          value: 0.25,
          duration: 7,
          ease: "power1.inOut",
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      const mouseInfluence =
        Math.sqrt(mouseX * mouseX + mouseY * mouseY) * 0.3;
